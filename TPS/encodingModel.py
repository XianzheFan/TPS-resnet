...
def verify():
    ...
    for index, (name, paths) in enumerate(images_map.items()):
        for path in paths:
            img = tps_photo(path)
            if(img_index % 2 == 0):
                if(img.all() == np.array([]).all()):
                    img = Image.open(path)
                    w, h = img.size
                    img = img.crop((int(w*0.25), int(h*0.25), int(w*0.75), int(h*0.75)))
                    img = img.resize((250, 250), Image.LANCZOS)
                    # img = img.resize((250, 250), Image.Resampling.LANCZOS)
                    img.save(f"test_debug_faces/{name}.A.jpg")
                else:
                    cv2.imwrite(f"test_debug_faces/{name}.A.jpg", img)
                img = Image.open(f"test_debug_faces/{name}.A.jpg")
                tensor = image_to_tensor(resize_image(img))
                save_tensor(tensor, os.path.join("testdata", f"{name}.A.pt"))
            else:
                if(img.all() == np.array([]).all()):
                    img = Image.open(path)
                    w, h = img.size
                    ...
                    img = img.resize((250, 250), Image.LANCZOS)
                    # img = img.resize((250, 250), Image.Resampling.LANCZOS)
                    img.save(f"test_debug_faces/{name}.B.jpg")
                else:
                    cv2.imwrite(f"test_debug_faces/{name}.B.jpg", img)
                img = Image.open(f"test_debug_faces/{name}.B.jpg")
                tensor = image_to_tensor(resize_image(img))  # debug_face里存的是250，但是tensor是80
                save_tensor(tensor, os.path.join("testdata", f"{name}.B.pt"))
            img_index += 1
    ...
    for i in range(0, 600):
        image_tensors = []
        A_tensor = load_tensor(test + str(i) + '.' + 'A.pt')
        B_tensor = load_tensor(test + str(i) + '.' + 'B.pt')
        image_tensors.append(A_tensor)
        image_tensors.append(B_tensor)
    
        predicted = recognize_model(torch.stack(image_tensors).to(device))

        for index in range(0, predicted.shape[0], 2):
            a = predicted[index] 
            b = predicted[index + 1] 
            # 判断同一人物的编码是否小于不同人物的编码
            diff = (a - b).pow(2).sum()   
            result_diff.append(diff.item())     
            if(diff > 0.4):
            # if(diff > 1.0):
                result.append(0)
            else:
    ...
    with open(filename, 'w') as f:
        for i in result:
            f.write(f'{str(i)}\r\n')
            print(i)
    with open('test_result_diff.txt', 'w') as f:
