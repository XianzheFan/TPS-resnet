...
#             img = Image.open(path)
#             img.save(f"test_debug_faces/{name}.{i}.jpg")  # 保存处理后的图片
#             img = Image.open(f"test_debug_faces/{name}.{i}.jpg")
#             tensor = image_to_tensor(resize_image(img))  # 使用与训练时相同的image_to_tensor函数
...
#         A_tensor = load_tensor(os.path.join(test, f"{name}.0.pt"))
#         B_tensor = load_tensor(os.path.join(test, f"{name}.1.pt"))
#         image_tensors = [A_tensor, B_tensor]
